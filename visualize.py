...
def hu_to_grayscale(volume, hu_min, hu_max):
    ...
    im_volume = (volume - mnval) / max(mxval - mnval, 1e-3) * 255
...
def visualize(path, destination, hu_min=DEFAULT_HU_MIN, hu_max=DEFAULT_HU_MAX,
    ...
    vol = nib.load(path + "/imaging.nii.gz")
    seg = nib.load(path + "/segmentation.nii.gz")
    spacing = vol.affine
...
if __name__ == '__main__':
    ...
    visualize(
        ...
        hu_min=args.lower_hu_bound,
        hu_max=args.upper_hu_bound,
        plane=args.plane
    )
